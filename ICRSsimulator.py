...
class ICRSsimulator(object):
    ...
    def showLocalMap(self, low_row, up_row, low_col, up_col):
        plt.imshow(self.ICRSmap[low_row:up_row+1, low_col:up_col+1, 0], cmap='gray', interpolation='none')
        plt.savefig('localMining.jpg')
        plt.clf()
    def saveLocalMap(self, low_row, up_row, low_col, up_col, episode, save_dir):
        plt.imshow(self.ICRSmap[low_row:up_row+1, low_col:up_col+1, 0], cmap='gray', interpolation='none')
        plt.savefig(os.path.join(save_dir, 'True Mining Episode {}.jpg'.format(episode)))
        plt.clf()
